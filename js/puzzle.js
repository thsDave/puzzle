...
$(document).ready(() => {
	...
	Toast.fire({
		...
		html: 'Pulsa en un cuadro y luego en otro para intercambiar sus posiciones 😎👍',
...
var rompecabezas = {
	...
	_cambiaBGP: function(id) {
		if (select == false) {
			...
			id_s = id;
			select = true;
			rompecabezas._get(id_s).style.boxShadow = '1px 1px 14px #F0DE1F,-1px -1px 14px #F0DE1F, 1px -1px 14px #F0DE1F,-1px 1px 14px #F0DE1F';
		}else {
    ...
    setTimeout(function() {
      if(fin) {
      	var Toast = Swal.mixin({
				...
				Toast.fire({
					title: '🥳😃🎉🎊🎈🎁😱😁🥳 Felicidades!! lograste resolverlo!',
					showDenyButton: true,
					confirmButtonText: `Volver a intentarlo`,
					denyButtonText: `Seleccionar otro puzzle`,
				}).then((result) => {
